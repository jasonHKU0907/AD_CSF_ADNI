...
outfile = dpath + 'Results/CNvsDementia/sb_rmMultiColinearity.csv'
outimg = dpath + 'Results/CNvsDementia/sb_rmMultiColinearity.png'
Imp_df = pd.read_csv(dpath + 'Results/CNvsDementia/sa_Importance.csv')
top_nb = get_imp_analy(Imp_df, top_prop)
Imp_df = Imp_df.iloc[:top_nb,:]
my_f_lst = Imp_df.Analytes.tolist()
mydf = pd.read_csv(dpath + 'Data/ADNI_CSF_DATA.csv', usecols = ['id', 'RID', 'DX3'] + my_f_lst)
