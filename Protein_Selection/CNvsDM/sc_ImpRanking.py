...
outfile = dpath + 'Results/CNvsDementia/sc_ReImportance.csv'
mydf = pd.read_csv(dpath + 'Data/ADNI_CSF_DATA.csv')
my_f_df = pd.read_csv(dpath + 'Results/CNvsDementia/sb_rmMultiColinearity.csv')
...
my_X = mydf[my_f_lst]
y = mydf.target_y
mykf = StratifiedKFold(n_splits = nb_folds, random_state = 2024, shuffle = True)
...
my_f_df.drop('TotalGain_cv', axis = 1, inplace = True)
my_imp_df = pd.merge(my_imp_df, my_f_df, how = 'left', on = 'Analytes')
