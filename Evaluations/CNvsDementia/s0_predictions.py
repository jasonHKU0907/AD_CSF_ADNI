...
outfile = dpath + 'Results/FinalModel/CNvsDementia/s0_predictions.csv'

target_df = pd.read_csv(dpath + 'Data/Data0921/ADNI_Target.csv', usecols=['RID', 'DXBLYES'])
target_df['target_y'] = target_df['DXBLYES'].copy()
target_df0, target_df1 = target_df.copy(), target_df.copy()

target_df0 = target_df0.loc[(target_df0.target_y == 'CN')|(target_df0.target_y == 'Dementia')]
target_df0.reset_index(inplace = True, drop = True)
target_df0['target_y'].replace(['CN', 'Dementia'], [0, 1], inplace = True)

target_df1 = target_df1.loc[target_df1.target_y == 'MCI']
target_df1.reset_index(inplace = True, drop = True)

pro_f_lst1 = ['ACHE', 'YWHAG', 'PCSK1', 'MMP10', 'IRF1']
pro_f_lst2 = ['YWHAZ', 'YWHAB', 'SMOC1']
...
mydf0 = pd.merge(target_df0, pro_df, how = 'left', on = ['RID'])
mydf0 = pd.merge(mydf0, bio_df, how = 'left', on = ['RID'])
mydf0 = pd.merge(mydf0, cov_df, how = 'left', on = ['RID'])

mydf1 = pd.merge(target_df1, pro_df, how = 'left', on = ['RID'])
mydf1 = pd.merge(mydf1, bio_df, how = 'left', on = ['RID'])
mydf1 = pd.merge(mydf1, cov_df, how = 'left', on = ['RID'])
...
pred_pro = get_pred_probs_all(mydf0, mydf1, pro_f_lst1, mykf, my_params, 'ProPanel')
pred_cov = get_pred_probs_all(mydf0, mydf1, cov_f_lst, mykf, my_params, 'Cov')
pred_pro_cov = get_pred_probs_all(mydf0, mydf1, pro_f_lst1+cov_f_lst, mykf, my_params, 'ProPanel_Cov')
pred_bio_comb = get_pred_probs_all(mydf0, mydf1, ['CSF_ABETA42', 'CSF_PTAU181', 'CSF_TTAU'], mykf, my_params, 'Comb_AB42_PT181_TTAU')

myout_df = pd.merge(target_df, pred_pro, how = 'left', on = 'RID')
myout_df = pd.merge(myout_df, pred_cov, how = 'left', on = 'RID')
...
for i in range(len(pro_f_lst1)):
    tmp_f = pro_f_lst1[i]
    tmp_df = mydf0[['target_y', tmp_f] + cov_f_lst]
    ...
    pred_pro_cov = get_pred_probs_all(mydf0, mydf1, [tmp_f]+cov_f_lst, mykf, my_params, tmp_f+'_Cov')
    myout_df = pd.merge(myout_df, pred_pro_cov, how='left', on='RID')
