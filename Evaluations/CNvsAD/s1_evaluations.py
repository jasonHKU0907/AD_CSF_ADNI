...
my_f_lst = mydf.columns.tolist()[4:]
outfile = dpath + 'Results/FinalModel/CNvsAD/s1_eval_CNvsAD.csv'
mydf['target_y'] = mydf['DX_AT_status1'].copy()
mydf = mydf.loc[(mydf.target_y == 'CN A-T-') | (mydf.target_y == 'A+T+')]
...
outfile = dpath + 'Results/FinalModel/CNvsAD/s1_eval_'+y_neg+'vs'+y_pos+'.csv'
...
results = [process(f, mydf) for f in my_f_lst]
