...
outfile = dpath + 'Results/FinalModel/CNvsAD/s0_predictions.csv'

target_df = pd.read_csv(dpath + 'Data/Data0921/ADNI_Target.csv', usecols=['RID', 'DX_AT_status1', 'DX_AT_status'])
target_df['target_y'] = target_df['DX_AT_status1'].copy()
target_df0, target_df1 = target_df.copy(), target_df.copy()

target_df0 = target_df0.loc[target_df0.target_y != 'other']
target_df0.reset_index(inplace = True, drop = True)
target_df0['target_y'].replace(['CN A-T-', 'A+T+'], [0, 1], inplace = True)

target_df1 = target_df1.loc[target_df1.target_y == 'other']
