...
for loocv_id in loocv_id_lst:
    traindf = mydf.loc[mydf.loocv_id != loocv_id]
    traindf.reset_index(inplace = True, drop = True)
    my_f_df = pd.read_csv(dpath + 'Results/CNvsAD/s00_Associations/Test_loocv'+str(loocv_id)+'.csv')
    my_f_df = my_f_df.loc[my_f_df.P_BH_adjust < 0.05]
    my_f_lst = my_f_df.Analytes
    results = Parallel(n_jobs=15)(delayed(process)(traindf, f) for f in my_f_lst)
    ...
    results.to_csv(dpath + 'Results/CNvsAD/s01_AUC_direct/Test_loocv' + str(loocv_id) + '.csv', index = False)
