...
for loocv_id in tqdm(loocv_id_lst):
    ...
    my_f_df = pd.read_csv(dpath + 'Results/CNvsAD/sb_rmMultiColinearity/Test_loocv' + str(loocv_id) + '.csv')
    ...
    tg_imp_df.to_csv(dpath + 'Results/CNvsAD/sc_Importance/Test_loocv' + str(loocv_id) + '.csv', index = False)
