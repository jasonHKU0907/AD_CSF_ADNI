...
for loocv_id in tqdm(loocv_id_lst):
    ...
    analy_auc_df = pd.read_csv(dpath + 'Results/CNvsDM/s01_AUC_direct/Test_loocv' + str(loocv_id) + '.csv')
    ...
    tg_imp_df.to_csv(dpath + 'Results/CNvsDM/sa_Importance/Test_loocv' + str(loocv_id) + '.csv', index = False)
