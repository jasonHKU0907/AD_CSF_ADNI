...
for loocv_id in tqdm(loocv_id_lst):
    ...
    Imp_df = pd.read_csv(dpath + 'Results/CNvsDM/sa_Importance/Test_loocv' + str(loocv_id) + '.csv')
    ...
    myout_df.to_csv(dpath + 'Results/CNvsDM/sb_rmMultiColinearity/Test_loocv' + str(loocv_id) + '.csv', index=False)
