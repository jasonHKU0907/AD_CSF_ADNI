...
for loocv_id in tqdm(loocv_id_lst):
    ...
    my_f_df = pd.read_csv(dpath + 'Results/CNvsDM/sc_Importance/Test_loocv' + str(loocv_id) + '.csv')
    ...
    results.to_csv(dpath + 'Results/CNvsDM/sd_sf_selection/Test_loocv' + str(loocv_id) + '.csv', index=False)
    print('finished')
