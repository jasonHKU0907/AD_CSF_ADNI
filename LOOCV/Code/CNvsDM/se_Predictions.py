...
for loocv_id in tqdm(loocv_id_lst):
    ...
    auc_imp_df = pd.read_csv(dpath + 'Results/CNvsDM/sd_sf_selection/Test_loocv' + str(loocv_id) + '.csv')
...
pred_df.to_csv(dpath + 'Results/CNvsDM/se0_loocv_prediction.csv', index = False)
...
out_df.to_csv(dpath + 'Results/CNvsDM/se1_loocv_evaluation.csv', index = False)
